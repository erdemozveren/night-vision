...
export default class Dataset {
  ...
  // Fetch data from WW
  async data() {
    const ds = await this.dc.ww.exec(
      'get-dataset',
      this.id
    )
    if (!ds) return
...
export class DatasetWW {
  ...
  static update_all(se, data) {
    for (var k in data) {
      ...
      const arr = se.data[id].data
      const iN = arr.length - 1
      const last = arr[iN]
  ...
  merge(data) {
    ...
    const tN = this.data[len - 1][0]
    const l = data.filter(x => x[0] < t0)
    const r = data.filter(x => x[0] > tN)
