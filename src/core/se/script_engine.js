...
class ScriptEngine {
  ...
  async exec_sel(delta) {
    ...
    const sel = Object.keys(delta).filter(x => x in this.map)
    ...
    for (var id in delta) {
      ...
      const props = this.map[id].src.props || {}
      for (var k in props) {
        if (k in delta[id]) {
  ...
  add_script(s) {
    ...
    const script = self.scriptLib.iScripts[s.type]
    if (!script) {
  ...
  update(candles, e) {
    ...
    const mfs1 = this.make_mods_hooks('pre_step')
    const mfs2 = this.make_mods_hooks('post_step')

    const step = (sel, unshift) => {
  ...
  std_inject(std) {
    const proto = Object.getPrototypeOf(std)
  ...
  async run(sel) {
    ...
    try {
      ...
      for (let i = start; i < ohlcv.length; i++) {
        ...
        for (const id of sel) this.map[id].env.step()
  ...
  start(ohlcv) {
    const depth = this.sett.script_depth
    return depth ?
  ...
  drain_queues() {
    ...
    // Check if there are any new settings deltas (...)
      ...
      while (this.update_queue.length) {
        const upd = this.update_queue.shift()
        this.update(...upd)
      }
    }
  }
  ...
  recalc_size() {
    while (true) {
      ...
      const lim = this.sett.ww_ram_limit
      if (lim && sz > lim) {
        this.limit_size()
      } else break
    }
  ...
  limit_size() {
    const dss = Object.values(this.data).map(x => ({
      id: x.id,
      t: x.last_upd
    }))
