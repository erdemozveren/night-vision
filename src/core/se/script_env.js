...
export default class ScriptEnv {
  ...
  make_box() {
    ...
    const code = this.src.code
    const proto = Object.getPrototypeOf(this.std)
    let std = ''
  ...
  prep(src) {
    ...
    // console.log('Before -----> \n', src)
    const h = this.src.type // TODO: add props here
    src = '\t\t  let _pref = `${_id}<-' + h + '<-`\n' + src

    FDEFS2.lastIndex = 0
    let call_id = 0 // Function call id (to make each call unique)
    const prefabs = self.scriptLib.prefabs
    ...
    do {
      ...
      if (m) {
        ...
        if (fkeyword === 'function') {
          ...
          const i1 = m.index
          const m0 = this.parentheses(m[0])
          const i2 = m.index + m0.length
          const args2 = this.args2(m[0])
          // Std Functions
          if (this.std[fname]) {
  ...
  postfix(src, m, call_id) {
    ...
    const target = this.get_args(this.fdef(m[2])).length
    const m0 = this.parentheses(m[0]) // First closed pair
    const args = this.get_args_2(m0)
  ...
  get_args(src) {
    const reg = this.regex_clone(FDEFS1)
    reg.lastIndex = 0
