...
export default class ScriptStd {
  ...
  _index_tracking() {
    ...
    for (var k of Object.getOwnPropertyNames(proto)) {
      ...
      const f = this._add_i(k, this[k].toString())
      if (f) this[k] = f
    }
  ...
  /**
     ...
     */
  prop(name, descr) {
    const props = this.env.src.props
    if (!(name in props)) {
  ...
  autoPrec() {
    ...
    const data = se.data.ohlcv.data
    const len = data.length
    const i0 = Math.max(0, len - 100)
    let max = 0
    for (var i = i0; i < len; i++) {
      const p = data[i]
      for (var k = 1; k < 5; k++) {
        const r = Utils.numberLR(p[k])[1]
        if (r > max) max = r
      }
    }
    return max
  ...
  _add_i(name, src) {
    const args = u.f_args(src)
    src = u.f_body(src)
    const src2 = u.wrap_idxs(src, 'this.')
    if (src2 !== src) {
  ...
  add(x, y, _id) {
    ...
    const id = this._tsid(_id, 'add')
    const x0 = this.na(x) ? NaN : (x.__id__ ? x[0] : x)
    const y0 = this.na(y) ? NaN : (y.__id__ ? y[0] : y)
  ...
  alma(src, len, offset, sigma, _id) {
    const id = this._tsid(_id, `alma(${len},${offset},${sigma})`)
  ...
  atr(len, _id, _tf) {
    const tfs = _tf || ''
    const id = this._tsid(_id, `atr(${len})`)
    const high = this.env.shared[`high${tfs}`]
    const low = this.env.shared[`low${tfs}`]
    const close = this.env.shared[`close${tfs}`]
    const tr = this.ts(0, id, _tf)
  ...
  /** Candles since the event occured (cond === true)
     ...
     */
  since(cond, _id) {
    const id = this._tsid(_id, 'since()')
    if (cond && cond.__id__) cond = cond[0]
    const s = this.ts(undefined, id)
    s[0] = cond ? 0 : s[1] + 1
    return s
  }
  ...
  /** Bollinger Bands
     ...
     */
  bb(src, len, mult, _id) {
    const id = this._tsid(_id, `bb(${len},${mult})`)
    const basis = this.sma(src, len, id)
    const dev = this.stdev(src, len, id)[0] * mult
    return [
      basis,
  ...
  bbw(src, len, mult, _id) {
    const id = this._tsid(_id, `bbw(${len},${mult})`)
    const basis = this.sma(src, len, id)[0]
    const dev = this.stdev(src, len, id)[0] * mult
  ...
  cci(src, len, _id) {
    ...
    const id = this._tsid(_id, `cci(${len})`)
    const ma = this.sma(src, len, id)
    const dev = this.dev(src, len, id)
    const cci = (src[0] - ma[0]) / (0.015 * dev[0])
  ...
  change(src, len = 1, _id) {
    const id = this._tsid(_id, `change(${len})`)
  ...
  /** Chande Momentum Oscillator
     ...
     */
  cmo(src, len, _id) {
    const id = this._tsid(_id, `cmo(${len})`)
    const mom = this.change(src, 1, id)

    const g = this.ts(mom[0] >= 0 ? mom[0] : 0.0, id + 'g', src.__tf__)
    const l = this.ts(mom[0] >= 0 ? 0.0 : -mom[0], id + 'l', src.__tf__)

    const sm1 = this.sum(g, len, id + '1')[0]
    const sm2 = this.sum(l, len, id + '2')[0]
  ...
  cog(src, len, _id) {
    const id = this._tsid(_id, `cmo(${len})`)
    const sum = this.sum(src, len, id)[0]
    let num = 0
    for (var i = 0; i < len; i++) {
      num += src[i] * (i + 1)
    }
  ...
  /** When one time-series crosses another
     ...
     */
  cross(src1, src2, _id) {
    const id = this._tsid(_id, 'cross')
    const x = (src1[0] > src2[0]) !== (src1[1] > src2[1])
  ...
  crossover(src1, src2, _id) {
    const id = this._tsid(_id, 'crossover')
    const x = (src1[0] > src2[0]) && (src1[1] <= src2[1])
    return this.ts(x, id, src1.__tf__)
  }
  ...
  crossunder(src1, src2, _id) {
    const id = this._tsid(_id, 'crossunder')
    const x = (src1[0] < src2[0]) && (src1[1] >= src2[1])
    return this.ts(x, id, src1.__tf__)
  }
  ...
  /** Sum of all elements of src
     ...
     */
  cum(src, _id) {
    const id = this._tsid(_id, 'cum')
    const res = this.ts(0, id, src.__tf__)
  ...
  /** Deviation from SMA
     ...
     */
  dev(src, len, _id) {
    const id = this._tsid(_id, `dev(${len})`)
    const mean = this.sma(src, len, id)[0]
    let sum = 0
    for (var i = 0; i < len; i++) {
  ...
  dmi(len, smooth, _id, _tf) {
    const id = this._tsid(_id, `dmi(${len},${smooth})`)
    const tfs = _tf || ''
    const high = this.env.shared[`high${tfs}`]
    const low = this.env.shared[`low${tfs}`]
    const up = this.change(high, 1, id + '1')[0]
    const down = this.neg(this.change(low, 1, id + '2'), id)[0]

    const plusDM = this.ts(100 * (
      this.na(up) ? NaN :
        (up > down && up > 0 ? up : 0)), id + '3', _tf
    )
    const minusDM = this.ts(100 * (
      this.na(down) ? NaN :
        (down > up && down > 0 ? down : 0)), id + '4', _tf
    )

    const trur = this.rma(this.tr(false, id, _tf), len, id + '5')
    const plus = this.div(
      this.rma(plusDM, len, id + '6'), trur, id + '8')
    const minus = this.div(
      this.rma(minusDM, len, id + '7'), trur, id + '9')
    const sum = this.add(plus, minus, id + '10')[0]
    const adx = this.rma(
  ...
  /** Exponential Moving Average with alpha = 2 / (y + 1)
     ...
     */
  ema(src, len, _id) {
    const id = this._tsid(_id, `ema(${len})`)
    const a = 2 / (len + 1)
    const ema = this.ts(0, id, src.__tf__)
    ema[0] = this.na(ema[1]) ?
  ...
  /** Test if "src" TS is falling for "len" candles
     ...
     */
  falling(src, len, _id) {
    const id = this._tsid(_id, `falling(${len})`)
    const bot = src[0]
  ...
  /** Highest value for a given number of candles back
     ...
     */
  highest(src, len, _id) {
    const id = this._tsid(_id, `highest(${len})`)
    let high = -Infinity
    for (var i = 0; i < len; i++) {
  ...
  highestbars(src, len, _id) {
    const id = this._tsid(_id, `highestbars(${len})`)
    let high = -Infinity
    let hi = 0
    for (var i = 0; i < len; i++) {
  ...
  /** Hull Moving Average
     ...
     */
  hma(src, len, _id) {
    const id = this._tsid(_id, `hma(${len})`)
    const len2 = Math.floor(len / 2)
    const len3 = Math.round(Math.sqrt(len))

    const a = this.mult(this.wma(src, len2, id + '1'), 2, id)
    const b = this.wma(src, len, id + '2')
    const delt = this.sub(a, b, id + '3')
  ...
  kc(src, len, mult, use_tr = true, _id, _tf) {
    ...
    const id = this._tsid(_id, `kc(${len},${mult},${use_tr})`)
    const tfs = _tf || ''
    const high = this.env.shared[`high${tfs}`]
    const low = this.env.shared[`low${tfs}`]
    const basis = this.ema(src, len, id + '1')

    const range = use_tr ?
      this.tr(false, id + '2', _tf) :
      this.ts(high[0] - low[0], id + '3', src.__tf__)
    const ema = this.ema(range, len, id + '4')
  ...
  kcw(src, len, mult, use_tr = true, _id, _tf) {
    const id = this._tsid(_id, `kcw(${len},${mult},${use_tr})`)
    const kc = this.kc(src, len, mult, use_tr, 'kcw', _tf)
  ...
  linreg(src, len, offset = 0, _id) {
    const id = this._tsid(_id, `linreg(${len})`)

    src.__len__ = Math.max(src.__len__ || 0, len)
    const lr = linreg(src, len, offset)
  ...
  /** Lowest value for a given number of candles back
     ...
     */
  lowest(src, len, _id) {
    const id = this._tsid(_id, `lowest(${len})`)
    let low = Infinity
    for (var i = 0; i < len; i++) {
  ...
  lowestbars(src, len, _id) {
    const id = this._tsid(_id, `lowestbars(${len})`)
    let low = Infinity
    let li = 0
    for (var i = 0; i < len; i++) {
  ...
  macd(src, fast, slow, sig, _id) {
    const id = this._tsid(_id, `macd(${fast}${slow}${sig})`)
    const fast_ma = this.ema(src, fast, id + '1')
    const slow_ma = this.ema(src, slow, id + '2')
    const macd = this.sub(fast_ma, slow_ma, id + '3')
    const signal = this.ema(macd, sig, id + '4')
    const hist = this.sub(macd, signal, id + '5')
  ...
  /** Money Flow Index
     ...
     */
  mfi(src, len, _id) {
    const id = this._tsid(_id, `mfi(${len})`)
    const vol = this.env.shared.vol
    const ch = this.change(src, 1, id + '1')[0]

    const ts1 = this.mult(vol, ch <= 0.0 ? 0.0 : src[0], id + '2')
    const ts2 = this.mult(vol, ch >= 0.0 ? 0.0 : src[0], id + '3')

    const upper = this.sum(ts1, len, id + '4')
    const lower = this.sum(ts2, len, id + '5')
  ...
  /** Momentum
     ...
     */
  mom(src, len, _id) {
    const id = this._tsid(_id, `mom(${len})`)
    return this.ts(src[0] - src[len], id, src.__tf__)
  }
  ...
  offset(src, num, _id) {
    ...
    const id = this._tsid(_id, `offset(${num})`)
    const out = this.ts(src, id)
    out.__offset__ = num
    return out
  }
  ...
  pivothigh(src, left, right, _id) {
    const id = this._tsid(_id, `pivothigh(${left},${right})`)
  ...
  pivotlow(src, left, right, _id) {
    const id = this._tsid(_id, `pivotlow(${left},${right})`)
  ...
  /** Test if "src" TS is rising for "len" candles
     ...
     */
  rising(src, len, _id) {
    const id = this._tsid(_id, `rising(${len})`)
    const top = src[0]
    for (var i = 1; i < len + 1; i++) {
      if (top <= src[i]) {
        return this.ts(false, id, src.__tf__)
  ...
  /** Exponentially MA with alpha = 1 / length
     ...
     */
  rma(src, len, _id) {
    const id = this._tsid(_id, `rma(${len})`)
    const a = len
    const sum = this.ts(0, id, src.__tf__)
    sum[0] = this.na(sum[1]) ?
      this.sma(src, len, id)[0] :
  ...
  /** Rate of Change
     ...
     */
  roc(src, len, _id) {
    const id = this._tsid(_id, `roc(${len})`)
    return this.ts(
  ...
  rsi(x, y, _id) {
    ...
    if (!this.na(y) && y.__id__) {
      ...
      const ch = this.change(x, 1, _id)[0]
      const pc = this.ts(Math.max(ch, 0), id + '1', x.__tf__)
      const nc = this.ts(-Math.min(ch, 0), id + '2', x.__tf__)
      const up = this.rma(pc, y, id + '3')[0]
      const down = this.rma(nc, y, id + '4')[0]
      rsi = down === 0 ? 100 : (
  ...
  sar(start, inc, max, _id, _tf) {
    ...
    const id = this._tsid(_id, `sar(${start},${inc},${max})`)
    const tfs = _tf || ''
    const high = this.env.shared[`high${tfs}`]
    const low = this.env.shared[`low${tfs}`]
    const close = this.env.shared[`close${tfs}`]

    const minTick = 0 //1e-7
    const out = this.ts(undefined, id + '1', _tf)
    const pos = this.ts(undefined, id + '2', _tf)
    const maxMin = this.ts(undefined, id + '3', _tf)
    const acc = this.ts(undefined, id + '4', _tf)
    const n = _tf ? out.__len__ - 1 : this.se.iter
    let prev
    let outSet = false
  ...
  /** Simple Moving Average
     ...
     */
  sma(src, len, _id) {
    const id = this._tsid(_id, `sma(${len})`)
    let sum = 0
    for (var i = 0; i < len; i++) {
      sum = sum + src[i]
    }
  ...
  stdev(src, len, _id) {
    ...
    const id = this._tsid(_id, `stdev(${len})`)
    const avg = this.sma(src, len, id)
    let sqd = 0
    for (var i = 0; i < len; i++) {
      const sum = sumf(src[i], -avg[0])
      sqd += sum * sum
    }
  ...
  stoch(src, high, low, len, _id) {
    const id = this._tsid(_id, `sum(${len})`)
    const x = 100 * (src[0] - this.lowest(low, len)[0])
    const y = this.highest(high, len)[0] - this.lowest(low, len)[0]
  ...
  supertrend(factor, atrlen, _id, _tf) {
    const id = this._tsid(_id, `supertrend(${factor},${atrlen})`)
    const tfs = _tf || ''
    const high = this.env.shared[`high${tfs}`]
    const low = this.env.shared[`low${tfs}`]
    const close = this.env.shared[`close${tfs}`]
    const hl2 = (high[0] + low[0]) * 0.5

    const atr = factor * this.atr(atrlen, id + '1', _tf)[0]

    const ls = this.ts(hl2 - atr, id + '2', _tf)
    const ls1 = this.nz(ls[1], ls[0])
    ls[0] = close[1] > ls1 ? Math.max(ls[0], ls1) : ls[0]

    const ss = this.ts(hl2 + atr, id + '3', _tf)
    const ss1 = this.nz(ss[1], ss)
    ss[0] = close[1] < ss1 ? Math.min(ss[0], ss1) : ss[0]

    const dir = this.ts(1, id + '4', _tf)
    dir[0] = this.nz(dir[1], dir[0])
    dir[0] = dir[0] === -1 && close[0] > ss1 ? 1 :
      (dir[0] === 1 && close[0] < ls1 ? -1 : dir[0])

    const plot = this.ts(dir[0] === 1 ? ls[0] : ss[0], id + '5', _tf)
  ...
  /** Symmetrically Weighted Moving Average
     ...
     */
  swma(src, _id) {
    const id = this._tsid(_id, 'swma')
    const sum = src[3] * this.SWMA[0] + src[2] * this.SWMA[1] +
  ...
  /** Creates a new Symbol.
     ...
     */
  sym(x, y = {}, _id) {
    const id = y.id || this._tsid(_id, 'sym')
    y.id = id
    if (this.env.syms[id]) {
  ...
  tr(fixnan = false, _id, _tf) {
    const id = this._tsid(_id, `tr(${fixnan})`)
    const tfs = _tf || ''
    const high = this.env.shared[`high${tfs}`]
    const low = this.env.shared[`low${tfs}`]
    const close = this.env.shared[`close${tfs}`]
  ...
  tsi(src, short, long, _id) {
    const id = this._tsid(_id, `tsi(${short},${long})`)
    const m = this.change(src, 1, id + '0')
    const m_abs = this.ts(Math.abs(m[0]), id + '1', src.__tf__)
    const tsi = (
  ...
  view(name, props = {}, _id) {
    if (!this.env.views[name]) {
      const view = new View(this, name, props)
  ...
  /** Volume Weighted Moving Average
     ...
     */
  vwma(src, len, _id) {
    const id = this._tsid(_id, `vwma(${len})`)
    const vol = this.env.shared.vol
    const sxv = this.ts(src[0] * vol[0], id + '1', src.__tf__)
    const res =
  ...
  /** Weighted moving average
     ...
     */
  wma(src, len, _id) {
    const id = this._tsid(_id, `wma(${len})`)
    let norm = 0
    let sum = 0
    for (var i = 0; i < len; i++) {
      const w = (len - i) * len
  ...
  /** Williams %R
     ...
     */
  wpr(len, _id, _tf) {
    const id = this._tsid(_id, `wpr(${len})`)
    const tfs = _tf || ''
    const high = this.env.shared[`high${tfs}`]
    const low = this.env.shared[`low${tfs}`]
    const close = this.env.shared[`close${tfs}`]

    const hh = this.highest(high, len, id)
    const ll = this.lowest(low, len, id)

    const res = (hh[0] - close[0]) / (hh[0] - ll[0])
