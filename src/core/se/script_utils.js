...
export function wrap_idxs(src, pre = '') {
  ...
  do {
    ...
    if (m) {
      ...
      const wrap = `${vname}[${pre}_i(${vindex}, ${vname})]`
...
export function ovf(obj, f) {
  ...
  for (var id in obj) {
    ...
    const vp = nw[id].vprops || {}
    if (!vp.full) {
      nw[id].data = f(obj[id].data)
    } else {
      nw[id].data = obj[id].data
    }
  }
  return nw
}
...
export function size_of_dss(data) {
  ...
  for (var id in data) {
    if (data[id].data && data[id].data[0]) {
      const s0 = size_of(data[id].data[0])
